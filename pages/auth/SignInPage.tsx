...
export const SignInPage: React.FC<SignInPageProps> = ({ onLogin }) => {
  return (
    <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
      ...
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        <form className="space-y-6" action="#" method="POST" onSubmit={(e) => { e.preventDefault(); onLogin('founder'); }}>
            ...
            <button type="submit" className="flex w-full justify-center rounded-md bg-green-accent px-3 py-1.5 text-sm font-semibold text-darker-gray">Sign in as Founder (Demo)</button>
            <button type="button" onClick={() => onLogin('investor')} className="flex w-full justify-center rounded-md bg-gold-accent px-3 py-1.5 text-sm font-semibold text-darker-gray">Sign in as Investor (Demo)</button>
        </form>
      </div>
    </div>
  );
